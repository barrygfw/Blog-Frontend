      ...
      >
        ...
        <div>
          ...
          <div
            style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px"
          >
            <span>登录次数:</span><span>{{ user.login_times }}</span>
          </div>
          <div
          ...
          >
            ...
            <span v-if="user.status === 1">
              <el-tag type="success" size="mini">已启用</el-tag>
              <el-button
                type="danger"
                size="mini"
                @click="enabledChange(0,user.id,index)"
                >禁用</el-button
              >
            </span>
            <span v-else>
              ...
              <el-button
                type="success"
                size="mini"
                @click="enabledChange(1,user.id,index)"
                >启用</el-button
              >
            </span>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      deleteUser(id) {
        ...
        this.$confirm('删除该用户, 是否继续?', '提示', {
          ...
          .then(() => {
            _this.loading = true;
            var ids = [id];
            deleteRequest('/user/delete', ids).then(
              resp => {
      ...
      enabledChange(enabled, id, index) {
        ...
        putRequest('/user/status', { status: enabled, id: id }).then(
          resp => {
            if (resp.status != 200) {
              _this.$message({ type: 'error', message: '更新失败!' });
              _this.loadOneUserById(id, index);
